...
class ConfigManager:
    ...
    # ==================================================================
    # 🎯 [확장성 확보] 통합 대시보드 및 멀티 에이전트 규격 일치화를 위한 인터페이스
    # ==================================================================
    def get_config(self, section=None):
        """통합 설정 데이터를 섹션별 또는 전체로 안전하게 반환
        
        Args:
            section (str, optional): 'api' 또는 'trading' 등 특정 섹션명
            
        Returns:
            dict: 매핑된 설정 데이터
        """
        # 1. 전체 설정을 하나의 통합 딕셔너리로 마스터 트리 구성
        master_config = {
            'api': self.get_api_config(),
            'trading': self.trading_config or {},
            'strategy': self.get_strategy_config(),
            'interval': self.get_interval(),
            'target_stocks': self.target_stocks
        }
        
        # 2. 특정 섹션만 정밀 타격하여 요청한 경우 해당 트리만 슬라이싱하여 반환
        if section:
            # 대시보드가 'api' 섹션을 요구하면 self.api_config 구조를 그대로 전달
            return master_config.get(section, {})
            
        return master_config

    # ==================================================================
    # 🎯 [확장성 확보] 대시보드 화면 연동을 위한 대상 종목 리스트 게터(Getter)
    # ==================================================================
    def get_target_stocks(self):
        """현재 로드되어 있는 감시 대상 종목 리스트를 안전하게 반환합니다.
        
        Returns:
            list: 종목 코드 문자열 리스트 (예: ['005930', '000660'])
        """
        # 내부에 저장된 target_stocks 리스트가 있으면 반환하고, 없으면 빈 리스트를 안전하게 반환하여
        # 대시보드 루프문이 NoneType 에러로 2차 붕괴하는 것을 원천 방어합니다.
        return self.target_stocks if self.target_stocks is not None else []